...
const Timer: React.FC = () => {
  ...
  const handleStartPress = async () => {
    ...
    if (isParent) {
      ...
      // await setDoc(doc(db, 'TimerControl', 'startTime'), { startTime });
  ...
  useEffect(() => {
    const startTimerListener = onSnapshot(doc(db, 'TimerControl', 'startTime'), (doc) => {
      const data = doc.data();
      if (data && data.startTime) {
        const delay = data.startTime - Date.now();
        if (delay > 0) {
          setTimeout(() => setCountdown(countdown), delay);
        }
      }
    });

    return () => startTimerListener();
  }, [db, countdown]);
