...
const Raids: React.FC = () => {
  ...
  if (Platform.OS === 'web') {
    return (
      <View style={styles.container}>
        <LoadScript googleMapsApiKey="AIzaSyAfRtyQRix_T4vKeHeJb_8YleHVwexKjXw">
          <GoogleMap
            mapContainerStyle={styles.map}
            center={currentLocation || { lat: 37.78825, lng: -122.4324 }}
            zoom={15}
            onClick={handleMapClick}
          >
            {currentLocation && (
              <Marker position={currentLocation} title="Ubicación Actual" />
            )}
            {destination && (
              <Marker position={destination} title="Destino" />
            )}
          </GoogleMap>
        </LoadScript>
  ...
  return (
    <View style={styles.webContainer}>
      <Text style={styles.webInfoText}>Para ver el mapa, por favor use la aplicación móvil.</Text>
    </View>
  );
};
